...
We didn't perform the realistic calculation because this calculation exists elsewhere, and is correct (\citealt{whatever}).
The point of this paper is conceptual, about information theory and theoretical questions for data analysis.

One question for the EPRV field is whether the future looks, qualitiatively, like the past?
...
Tellurics, for example, might not matter, and it might be possible to make absolute (rather than continuum-normalized) flux measurements.
These changes wouldn't change anything fundamental here.
But it might change how we design our pipelines and make our measurments in detail.
...
One interesting difference though is that when we measure extragalactic redshifts---or when ESA \textsl{Gaia} measures stellar radial velocities (\citealt{gaiarvs})---the intention is to measure \emph{absolute} radial velocities relative to the Milky Way (or Solar System) reference frame, not just relative velocities.
...
That's hard at the present day, and limits the precision of all absolute velocity measurements, in both Galactic and extragalactic contexts.
