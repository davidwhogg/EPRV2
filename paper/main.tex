...
The possible existence of these lines was one of the motivations for the \project{wobble} project (\citealt{wobble}.
...
BEDELL: DO WE NEED TO SAY MORE ALONG THESE LINES? Like contextualize with existing formulations? Or give our result in words, like ``the inner product of the derivative of blah with blah''?

Most extreme-precision radial-velocity (EPRV) spectrograph pipelines in fact measure radial velocities by a particular standard method, which involves cross-correlation with a binary mask, followed by fitting a Gaussian (or other smooth function) to that cross-correlation.
...
Speaking of assumptions: We make many for all of our results!
There are many respects in which real EPRV data will not satisfy our assumptions in detail.
In \secref{sec:sensitivity} we show hints that indeed our results are sensitive to the important assumptions about tellurics and spectral variability.
We do not, however, perform a full, quantitative sensitivity analysis.
This might be a good idea for planning hardware and surveys, and there are some examples of work along these lines in the literature (\citealt{halverson}).

In addition, another unreality of this work is that we use a completely fake spectrum in an arbitrary wavelength range at hard-set resolution.
The real precision of a real spectrograph observing a real star depends on the true spectrum and the true resolution and noise.
We didn't perform the realistic calculation because such calculations are very context-dependent, they exist elsewhere (\citealt{whatever}), and they are correct.
The point of this paper is conceptual, about information theory and theoretical questions for data analysis.

One question for the EPRV field is whether the future looks, qualitatively, like the past?
If EPRV moves to space (for example, \citealt{whatever}), there might be huge advantages in terms of spectral quality and calibration.
Tellurics, for example, might not matter in space (although interstellar lines will still be present), and it might be possible to make absolute (rather than continuum-normalized) flux measurements.
These changes wouldn't change anything fundamental here.
But it might change how we design our pipelines and make our measurements in detail.

Nothing about the analysis here depends on high resolution, however.
Measurements of quasar or galaxy redshifts at lower resolution are subject to the same bounds.
One interesting difference though is that when we measure extragalactic redshifts---or when ESA \project{Gaia} measures stellar radial velocities (\citealt{gaiarvs})---the intention is to measure \emph{absolute} radial velocities relative to the Milky Way (or Solar System) reference frame, not just relative velocities (such as velocity differences from epoch to epoch).
The measurement of an absolute velocity depends not just on a spectral template, but on an \emph{accurate} spectral template, that is computed in a suitable rest frame.
That's hard at the present day, because of details of stellar photospheres and convective shifts, and so on (\citealt{something}).
These absolute-spectral-template issues limit the precision of all absolute velocity measurements, in both Galactic and extragalactic contexts.
The work we have presented here addresses only the precision of relative radial-velocity measurements and not the accuracy of any absolute measurement.

HOGG: In the above, make sure to comment on the conceptual point of whether you are measuring a velocity wrt the spectrograph, or a velocity wrt the Solar-System barycenter (as \project{EXPRES} does). This distinction is important to the design of extraction pipelines, but doesn't change any of our story here.
...
Finally, and most importantly, all results on radial-velocity measurement precision, and all of the methods for actually making measurements, depend on the stellar spectrum being (even in fine details) invariant with time.
