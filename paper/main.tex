...
% - I think we should only measure Doppler shift, and never RV. Right??
% - Related: we should cite the paper from Wright's group about the relativistically exact RV expressions?
% - Cite Vakili et al for the parabola trick, maybe?
% - Make the comment that the detailed noise model doesn't matter -- the CCF RVs are as good as the ML RVs. Probably the noise model only matters in that it can be used to censor or mask bad data.
% - Comment on how to estimate RV uncertainties & the accuracy of those estimates for each technique??
...
\begin{description}
    ...
    We will assume that the star has been observed many times ($N\gg1$). Preferably these observations were taken on different nights, but it is really only the \emph{number} of observations that is required.
