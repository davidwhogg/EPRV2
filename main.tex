...
% to-do list
% ----------
% - Synchronize to a git repository, and synchronize also the notebooks.
% - Look for all instances of HOGG, MB, and CITE.
...
% headers and date
\renewcommand{\today}{2022 January} % hurt me
...
HOGG: Maybe show that the wrong template (like missing some lines) gives less-good RVs.
...
In the toy-data system, we made the binary mask HOGG HOW??
The binary mask created this way is shown in \figref{fig:binarymask}.
...
HOGG: Note that the binary-mask operations are very sensitive to edge effects. In what we did above, we removed lines near the edges of the observed spectral range, so that our results aren't contaminated by variations resulting from lines entering and leaving the spectral range.
...
HOGG: Demonstration with the toy data: FT the data, filter, shift and average, get RVs, shift and average again, and so on.
...
Here we perform experiments in which we make the data weakly incompatible with the assumptions and look at how the radial-velocity measurements are affected.
